...
from web.pdf.io.filter.stream_decode_util import decode_stream
from web.pdf.io.read.tokenize.high_level_tokenizer import HighLevelTokenizer
from web.pdf.io.read.types import Dictionary, List, Name, Reference, Stream
from web.pdf.pdf.document.document import Document
from web.pdf.pdf.xref.xref import XREF
...
class StreamXREF(XREF):
    ...
    Cross-reference streams provide the following advantages: • A more compact
    representation of cross-reference information • The ability to access compressed
    objects that are stored in object streams (see 7.5.7, "Object Streams") and to
    allow new cross-reference entry types to be added in the future Cross-reference
    streams are stream objects (see 7.3.8, "Stream Objects"), and contain a dictionary
    and a data stream. Each cross-reference stream contains the information equivalent
    to the cross-reference table  (see 7.5.4, "Cross-Reference Table") and trailer (see
    7.5.5, "File Trailer") for one cross-reference section.
    """
            ...
            for i in range(0, length):
                ...
                if widths[0] > 0:
                    ...
                    for j in range(0, widths[0]):
                        type_ = (type_ << 8) + (xref_stream_decoded_bytes[bptr] & 0xFF)
                        bptr += 1
                ...
                if type_ == 0:
                    # type      : The type of this entry, which shall be 0. Type 0
                    #             entries define the linked list of free objects
                    #             (corresponding to f entries in a cross-reference
                    #             table).
                    # field2    : The object number of the next free object
                    # field3    : The generation number to use if this object number
                    #             is used again
                ...
                if type_ == 1:
                    # Type      : The type of this entry, which shall be 1. Type 1
                    #             entries define objects that are in use but are not
                    #             compressed (corresponding to n entries in a
                    #             cross-reference table).
                    # field2    : The byte offset of the object, starting from the
                ...
                if type_ == 2:
                    # Type      : The type of this entry, which shall be 2. Type 2
                    #             entries define compressed objects.
                    # field2    : The object number of the object stream in which this
                    #             object is stored. (The generation number of the
                    #             object stream shall be implicitly 0.)
