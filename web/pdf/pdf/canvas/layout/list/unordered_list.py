...
from web.pdf.pdf.canvas.layout.list.list import List
...
class UnorderedList(List):
    ...
    @staticmethod
    def _determine_level(layout_element: LayoutElement) -> int:
        level = 0
        e = layout_element
    ...
    def _get_bullet_layout_element(
        ...
        if isinstance(item, List):
            return ChunkOfText(" ")
        return ChunkOfText(
            text=self._get_bullet_text(item),
            font_size=self.get_font_size(),
            font_color=X11Color("Black"),
            font="Zapfdingbats",
        )

    def _get_bullet_text(self, item: LayoutElement) -> str:
