...
from web.pdf.pdf.canvas.color.color import Color, HSVColor, RGBColor
from web.pdf.pdf.canvas.geometry.rectangle import Rectangle
from web.pdf.pdf.canvas.layout.shape.disconnected_shape import (
    DisconnectedShape,
)
...
class GradientColoredDisconnectedShape(DisconnectedShape):
    """This class represents a generic disconnected shape (specified by a List of
    lines), that will be colored according to a gradient.
    ...
    class GradientType(enum.Enum):
        """This enumeration represents the type of gradient that will be applied to
        this DisjointShape."""
    ...
    def _paint_content_box(self, page: Page, bounding_box: Rectangle) -> None:
        ...
        min_x: Decimal = min([min(x[0][0], x[1][0]) for x in self._lines])
        min_y: Decimal = min([min(x[0][1], x[1][1]) for x in self._lines])
        max_x: Decimal = max([min(x[0][0], x[1][0]) for x in self._lines])
        max_y: Decimal = max([max(x[0][1], x[1][1]) for x in self._lines])

        # we need to calculate the norm (the max distance between a line and the origin
        # of the gradient)
        n: Decimal = Decimal(1)
        ...
        if self._gradient_type == GradientColoredDisconnectedShape.GradientType.RADIAL:
            ...
            n = Decimal(
                max(
                    [
                        math.sqrt((x[0][0] - mid_x) ** 2 + (x[0][1] - mid_y) ** 2)
        ...
        for line in self._lines:
            ...
            if (
                ...
                d = Decimal(
                    math.sqrt((line[0][0] - min_x) ** 2 + (line[0][1] - min_y) ** 2)
                )
            if (
                self._gradient_type
                == GradientColoredDisconnectedShape.GradientType.HORIZONTAL
            ):
                d = Decimal(line[0][0] - min_x)
            if (
                self._gradient_type
                == GradientColoredDisconnectedShape.GradientType.RADIAL
            ):
                d = Decimal(
                    math.sqrt((line[0][0] - mid_x) ** 2 + (line[0][1] - mid_y) ** 2)
                )
            if (
                self._gradient_type
                == GradientColoredDisconnectedShape.GradientType.VERTICAL
            ):
                d = Decimal(line[0][1] - min_y)

            # calculate color based on the distance between this line and the origin
            # of the gradient
