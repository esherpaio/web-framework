...
from web.pdf.io.read.types import AnyPDFType, Reference


class WriteTransformerState:
    """This class represents all the meta-information used in the process of persisting
    a PDF document.
    ...
    def __init__(
        ...
        self.root_object: Optional[AnyPDFType] = (
            root_object  # this is the root object (PDF)
        )
        self.indirect_objects_by_id: typing.Dict[
            int, AnyPDFType
        ] = {}  # these are the indirect objects (by id)
        self.indirect_objects_by_hash: typing.Dict[
            int, typing.List[AnyPDFType]
        ] = {}  # these are the indirect objects (by hash)
        ...
        self.apply_font_subsetting: bool = (
            False  # whether to apply Font subsetting or not
        )
...
class Transformer:
    ...
    It allows you to add child BaseWriteTransformer implementations to handle specific
    cases, such as persisting Image objects, or Dictionary objects, etc.
    """
    ...
    @staticmethod
    def _end_object(context: Optional[WriteTransformerState]):
        """This function writes the "endobj" bytes whenever a direct object needs to be
        closed."""
    ...
    @staticmethod
    def _start_object(
        object_to_transform: AnyPDFType, context: Optional[WriteTransformerState]
    ):
        """This function starts a new direct object by writing its reference number
        followed by "obj" (e.g. "12 0 obj").
    ...
    def add_child_transformer(self, handler: "Transformer") -> "Transformer":
        ...
        `can_be_transformed` method should return True if the child
        WriteBaseTransformer can handle the object to be transformed. The first child
        to return True will have its `transform` method called. This function returns
        self.
        """
    ...
    def can_be_transformed(self, any_: AnyPDFType):
        """This function returns True if this WriteBaseTransformer can transform the
        input object, false otherwise."""

        return False

    def get_reference(
        self, object_: AnyPDFType, context: WriteTransformerState
    ) -> Reference:
        """This function builds a Reference for the input object References are re-used
        whenever possible (hashing is used to detect duplicate objects)"""

        is_unique: bool = False
        try:
            is_unique = object_.is_unique()
        except:  # noqa: E722
            pass
        ...
        obj_hash: int = self._hash(object_)
        ...
        if obj_hash in context.indirect_objects_by_hash:
            context.indirect_objects_by_hash[obj_hash].append(object_)
        else:
            context.indirect_objects_by_hash[obj_hash] = [object_]

        # insert into context.indirect_objects_by_id
        context.indirect_objects_by_id[obj_id] = object_

        # return
        return ref

    def get_root_transformer(self) -> "Transformer":
        """This function returns the WriteBaseTransformer at the root of this
        WriteBaseTransformer hierarchy.

        This allows child WriteBaseTransformer implementations to call the root
        transformer. This is useful for instance when transforming a dictionary, where
        each key/value would then be transformed by delegating the call to the root
    ...
    def transform(
        ...
        """This method writes an object (of type AnyPDFType) to a byte stream
        (specified in the WriteTransformerState)"""

        # transform object
        return_value = None
        for h in self._handlers:
            if h.can_be_transformed(object_to_transform):
                return_value = h.transform(object_to_transform, context=context)
                break
