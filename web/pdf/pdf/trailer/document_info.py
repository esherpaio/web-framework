...
from web.pdf.io.write.conformance_level import ConformanceLevel
...
class DocumentInfo:
    ...
    def check_signatures() -> bool:
        """This method verifies the signatures in the Document, it returns True if the
        signatures match the digest of the Document (or if the Document has no
    ...
    def get_conformance_level_upon_create(self) -> typing.Optional[ConformanceLevel]:
        """This function returns the ConformanceLevel that was set for writing
        operations upon creating the Document instance.

        This allows the user to specify whether they want to enable things like tagging.
        A document that was already tagged, and read will of course remain tagged.
        A document that was not tagged, will similarly not magically be provided with
        tags.
        This ConformanceLevel only applies to Document instances that were created.
    ...
    def get_creator(self) -> Optional[str]:
        ...
        name of the conforming product that created the original document from which it
        was converted."""
    ...
    def get_ids(self) -> Optional[List[str]]:
        ...
        trailer dictionary (see 7.5.5, “File Trailer”).

        The ID entry is optional but should be used. The value of this entry shall be
        an array of two byte strings. The first byte string shall be a permanent
        identifier based on the contents of the file at the time it was originally
        created and shall not change when the file is incrementally updated. The second
        byte string shall be a changing identifier based on the file’s contents at the
        time it was last updated. When a file is first written, both identifiers shall
        be set to the same value. If both identifiers match when a file reference is
        resolved, it is very likely that the correct and unchanged file has been found.
        If only the first identifier matches, a different version of the correct file
        has been found.
        """
    ...
    def get_modification_date(self) -> Optional[str]:
        """Required if PieceInfo is present in the document catalogue; otherwise
        optional; PDF 1.1) The date and time the document was most recently modified, in
        human- readable form (see 7.9.4, “Dates”)."""
    ...
    def get_optional_content_group_names(self) -> typing.List[str]:
        """This function returns the name(s) of the optional content group(s), suitable
        for presentation in a reader’s user interface."""
    ...
    def get_producer(self) -> Optional[str]:
        """(Optional) If the document was converted to PDF from another format, the
        name of the conforming product that converted it to PDF."""
    ...
    def has_optional_content(self) -> bool:
        """Optional content (PDF 1.5) refers to sub-clauses of content in a PDF
        document that can be selectively viewed or hidden by document authors or
        consumers.

        This capability is useful in items such as CAD drawings, layered artwork, maps,
    ...
    def has_signatures(self) -> bool:
        """This function returns True if this Document has signatures, False
        otherwise."""

        # A PDF document may contain the following standard types of signatures:
        # 1. One or more approval signatures. These signatures appear in signature form
        # fields (see 12.7.4.5, “Signature Fields”).
        ...
        # 2. At most one certification signature (PDF 1.5). The signature dictionary of
        # a certification signature shall be the value of a signature field and shall
        # contain a ByteRange entry. It may also be referenced from the DocMDP entry
        # in the permissions dictionary (see 12.8.4, “Permissions”).
        ...
        # 3. At most two usage rights signatures (PDF 1.5). Its signature dictionary
        # shall be referenced from the UR3 (PDF 1.6) entry in the permissions
        # dictionary, whose entries are listed in Table 258, (not from a signature
        # field).
...
class XMPDocumentInfo(DocumentInfo):
    ...
    def get_metadata_date(self) -> Optional[str]:
        """
        (Optional) The date and time the metadata for this document was created, in
        human- readable form (see 7.9.4, “Dates”).
        """
    ...
    def get_publisher(self) -> Optional[str]:
        """(Optional; PDF 1.1) The name of the person/software who/which published the
        document."""
