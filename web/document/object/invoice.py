...
from .._utils import number_to_str, save_pdf
...
def _build_order_lines(order: Order) -> list[FixedColumnWidthTable]:
    ...
    for num, order_line in enumerate(order.lines):
        ...
        price_text = f"{number_to_str(order_line.total_price)} {order.currency_code}"
    ...
    subtotal_value_text = f"{number_to_str(order.subtotal_price)} {order.currency_code}"
    ...
    subtotal_value_text = f"{number_to_str(order.discount_price)} {order.currency_code}"
    subtotal_value_p = TextPG(subtotal_value_text)
    cells.append(TableCell(subtotal_head_p, col_span=h_count - 1))
    cells.append(TableCell(subtotal_value_p, col_span=1))
    ...
    subtotal_value_text = f"{number_to_str(order.shipment_price)} {order.currency_code}"
    subtotal_value_p = TextPG(subtotal_value_text)
    cells.append(TableCell(subtotal_head_p, col_span=h_count - 1))
    cells.append(TableCell(subtotal_value_p, col_span=1))
    ...
    vat_value_text = f"{number_to_str(order.vat_amount)} {order.currency_code}"
    ...
    total_value_text = f"{number_to_str(order.total_price_vat)} {order.currency_code}"
