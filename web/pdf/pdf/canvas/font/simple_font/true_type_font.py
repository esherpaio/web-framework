...
from web.pdf.pdf.canvas.font.composite_font.cid_font_type_2 import (
    CIDType2Font,
)
from web.pdf.pdf.canvas.font.composite_font.font_type_0 import Type0Font
from web.pdf.pdf.canvas.font.simple_font.font_type_1 import Type1Font
...
class TrueTypeFont(Type1Font):
    """A TrueType font dictionary may contain the same entries as a Type 1 font
    dictionary (see Table 111), with these differences:

    •   The value of Subtype shall be TrueType. •   The value of Encoding is subject to
    limitations that are described in 9.6.6,     "Character Encoding". •   The value of
    BaseFont is derived differently. The PostScript name for the value     of BaseFont
    may be determined: •   If the TrueType font program's “name” table contains a
    PostScript name, it     shall be used. •   In the absence of such an entry in the
    “name” table, a PostScript name shall be     derived from the name by which the
    font is known in the host operating system.     On a Windows system, the name shall
    be based on the lfFaceName field in a     LOGFONT structure; in the Mac OS, it
    shall be based on the name of the FOND     resource. If the name contains any
    SPACEs, the SPACEs shall be removed.
    """
    ...
    def _get_font_descriptor(ttf_font_file: TTFont) -> Dictionary:
        ...
        font_descriptor[Name("FontName")] = String(
            TrueTypeFont._get_base_font(ttf_font_file)
        )
        ...
        font_descriptor[Name("ItalicAngle")] = bDecimal(
            ttf_font_file["post"].italicAngle
        )
        font_descriptor[Name("Ascent")] = bDecimal(
            ttf_font_file["hhea"].ascent / units_per_em * 1000
        )
        font_descriptor[Name("Descent")] = bDecimal(
            ttf_font_file["hhea"].descent / units_per_em * 1000
        )
    ...
    def true_type_font_from_file(
        ...
        if len(glyph_order) >= 256:
            ...
            type_0_font["DescendantFonts"][0]["FontDescriptor"][Name("FontFile2")] = (
                TrueTypeFont._get_font_file_stream(font_file_bytes)
            )
            return type_0_font
        ...
        units_per_em: bDecimal = bDecimal(ttf_font_file["head"].unitsPerEm)
        if cmap is not None:
            font[Name("FirstChar")] = bDecimal(
                min([cid for cid, _ in enumerate(glyph_order)])
            )
            font[Name("LastChar")] = bDecimal(
                max([cid for cid, _ in enumerate(glyph_order)])
            )
