...
from doc.io.read.types import AnyPDFType, Dictionary, List, Name, Reference, Stream
...
class StreamTransformer(Transformer):
    """This implementation of WriteBaseTransformer is responsible for writing Stream
    objects."""
    ...
    def can_be_transformed(self, any_: AnyPDFType):
        """This function returns True if the object to be converted represents a Stream
        object."""

        return isinstance(any_, Stream)
    ...
    def transform(
        ...
        assert (
            context is not None
        ), "context must be defined in order to write Stream objects."
        assert (
            context.destination is not None
        ), "context.destination must be defined in order to write Stream objects."
        assert isinstance(
