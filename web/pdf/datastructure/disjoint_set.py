...
class DisjointSet:
    ...
    data structure or merge-find set, is a data structure that stores a collection of
    disjoint (non-overlapping) sets.

    Equivalently, it stores a partition of a set into disjoint subsets. It provides
    operations for adding new sets, merging sets (replacing them by their union), and
    finding a representative member of a set. The last operation allows to find out
    efficiently if any two elements are in the same or different sets.
    """
    ...
    def union(self, x: Any, y: Any) -> "DisjointSet":
        """Mark two elements in this DisjointSet as equivalent, propagating the
        equivalence throughout the DisjointSet."""
