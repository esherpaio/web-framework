...
class AdobeGlyphList:
    """The Adobe Glyph List (AGL) is a mapping of 4,281 glyph names to one or more
    Unicode characters.

    Its purpose is to provide an implementation guideline for consumers of fonts
    (mainly software applications); it lists a variety of standard names that are given
    to glyphs that correspond to certain Unicode character sequences. The AGL is
    maintained by Adobe Systems. For producers of fonts, Adobe suggests a more limited
    set of names, the Adobe Glyph List for New Fonts (AGLFN), based on an earlier
    version of the AGL. Names not in the AGLFN are to be constructed by standard
    heuristics described in Unicode and Glyph Names. AGL and AGLFN, along with related
    resources, are currently maintained and available at the AGL & AGLFN open source
    project.
    """
