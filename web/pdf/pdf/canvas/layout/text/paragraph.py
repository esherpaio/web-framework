...
from web.pdf.pdf.canvas.font.glyph_line import GlyphLine
from web.pdf.pdf.canvas.geometry.rectangle import Rectangle
from web.pdf.pdf.canvas.layout.hyphenation.hyphenation import Hyphenation
from web.pdf.pdf.canvas.layout.layout_element import Alignment
from web.pdf.pdf.canvas.layout.text.line_of_text import LineOfText
...
class Paragraph(LineOfText):
    ...
    is a self-contained unit of discourse in writing dealing with a particular point or
    idea.
    ...
    def __init__(
        ...
        font_size: Decimal = Decimal(10),
        text_alignment: Alignment = Alignment.LEFT,
        vertical_alignment: Alignment = Alignment.TOP,
        horizontal_alignment: Alignment = Alignment.LEFT,
        font_color: Color = HexColor("444444"),
    ...
    def _get_content_box(self, available_space: Rectangle) -> Rectangle:
        ...
        # if this Paragraph previously fit in that box, and nothing has changed (e.g.
        # font_size, leading, etc) then this Paragraph still fits inside that box
        if (
        ...
        # if the hash has changed, it implies some attributes that are relevant to
        # layout have changed we need to recalculate the content_box, otherwise we can
        # just skip this and return the previously calculated content_box
        ...
        #  it is customary to treat the last line of a paragraph separately by simply
        #  left or right aligning it, depending on the language direction.
        if (
    ...
    def _split_text(self, bounding_box: Rectangle) -> typing.List[str]:
        ...
        for i, w in enumerate(words):
            ...
            potential_width = GlyphLine.from_str(
                potential_text, self._font, self._font_size
            ).get_width_in_text_space()
            remaining_space_in_box: Decimal = round(
                bounding_box.width - potential_width, 2
            )

            # IF there is space left over, we add the word to the lines of text
            ...
            # BUT perhaps we can hyphenate the word
                ...
                for hi in range(1, len(hyphenated_word_parts)):
                    potential_text_after_hyphenation = (
                        potential_text
                        + "".join([x for x in hyphenated_word_parts[0:hi]])
                        + "-"
                    )
                    potential_width = GlyphLine.from_str(
                        potential_text_after_hyphenation, self._font, self._font_size
                    ).get_width_in_text_space()
                    remaining_space_in_box = round(
                        bounding_box.width - potential_width, 2
                    )

                    if remaining_space_in_box > Decimal(0):
                        hyphenation_split_index = hi
                    else:
                        break
                ...
                if len(lines_of_text) > 0:
                    ...
                    lines_of_text[-1] += (
                        "".join(
                            [
                                x
                                for x in hyphenated_word_parts[
                                    0:hyphenation_split_index
                                ]
                            ]
                        )
                        + "-"
                ...
                lines_of_text.append(
                    "".join(
                        [x for x in hyphenated_word_parts[hyphenation_split_index:]]
                    )
                )
