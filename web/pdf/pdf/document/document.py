...
from web.pdf.io.read.types import Dictionary, List, Name, Stream, String
from web.pdf.io.write.conformance_level import ConformanceLevel
from web.pdf.pdf.canvas.layout.annotation.link_annotation import (
    DestinationType,
)
from web.pdf.pdf.document.name_tree import NameTree
from web.pdf.pdf.page.page import Page
from web.pdf.pdf.trailer.document_info import DocumentInfo, XMPDocumentInfo
from web.pdf.pdf.xref.plaintext_xref import PlainTextXREF
...
class Document(Dictionary):
    ...
    def add_embedded_file(self, file_name: str, file_bytes: bytes) -> "Document":
        ...
        and the PDF file is archived or transmitted, some provision should be made to
        ensure that the external references will remain valid.

        One way to do this is to arrange for copies of the external files to accompany
        the PDF file. Embedded file streams (PDF 1.3) address this problem by allowing
        the contents of referenced files to be embedded directly within the body of the
        PDF file. This makes the PDF file a self-contained unit that can be stored or
        transmitted as a single entity. (The embedded files are included purely for
        convenience and need not be directly processed by any conforming reader.) This
        method embeds a file (specified by its name and bytes) into this Document
        """
    ...
    def add_outline(
        ...
        display on the screen, allowing the user to navigate interactively from one
        part of the document to another.

        The outline consists of a tree-structured hierarchy of outline items (sometimes
        called bookmarks), which serve as a visual table of contents to display the
        document’s structure to the user. This function adds an outline to this
        Document
        """
    ...
    def get_embedded_file(self, file_name: str) -> typing.Optional[bytes]:
        ...
        transmitted as a single entity. (The embedded files are included purely for
        convenience and need not be directly processed by any conforming reader.) This
        method returns the embedded file specified by the given file_name
        """
    ...
    def get_embedded_files(self) -> typing.Dict[str, bytes]:
        ...
        transmitted as a single entity. (The embedded files are included purely for
        convenience and need not be directly processed by any conforming reader.) This
        method returns all embedded files, as a dictionary of names unto bytes
        """
    ...
    def has_outlines(self) -> bool:
        ...
        The outline consists of a tree-structured hierarchy of outline items (sometimes
        called bookmarks), which serve as a visual table of contents to display the
        document’s structure to the user. This function returns True if this Document
        has outlines, false otherwise
        """
    ...
    def insert_page(self, page: Page, index: typing.Optional[int] = None) -> "Document":
        """This method appends a page (from another Document) to this Document at a
        given index."""
        ...
        if "Pages" not in self["XRef"]["Trailer"]["Root"]:
            ...
            self["XRef"]["Trailer"][Name("Root")][Name("Pages")][Name("Count")] = (
                bDecimal(0)
            )
