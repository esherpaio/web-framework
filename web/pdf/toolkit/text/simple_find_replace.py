...
from web.pdf.pdf.canvas.layout.annotation.redact_annotation import (
    RedactAnnotation,
)
...
class SimpleFindReplace:
    """This class implements a (single word) find/replace functionality for PDF
    documents.

    This class uses RegularExpressionTextExtraction to find the text, RedactAnnotation
    to remove the text, and Paragraph to re-insert replacement text.
    """
    ...
    def sub(
        ...
        :param page_range:                      the pages on which to apply find/replace
        :param repl_font:                       the Font to be used to insert the replacement text
        :param repl_font_color:                 the font_color to be used to isnert the replacement text
        :param repl_font_size:                  the font_size to be used to insert the replacement text
        :param repl_font_horizontal_alignment   the horizontal_alignment of the replacement text
        :return:                                the PDF Document
        """

        from web.pdf.toolkit import PDFMatch, RegularExpressionTextExtraction
        ...
        if page_range is None:
            ...
            page_range = [x for x in page_range if 0 <= x < number_of_pages]
        for page_nr in page_range:
            ...
            for pdf_match in matches_per_page[page_nr]:
                ...
                bb_h: Decimal = max(
                    [
                        x.get_height()
                        for x in pdf_match.get_bounding_boxes()
                        if x.get_y() == bb_y
                    ]
                )
                bb_w: Decimal = sum(
