...
from web.pdf.pdf.canvas.color.color import Color
...
from web.pdf.pdf.canvas.event.event_listener import Event, EventListener
...
class PDFMatch:
    ...
    It has convenience methods to allow the user to extract information about the text
    that was matched, as well as the location (on the page) of the match.
    """
    ...
    def end(self, __group: typing.Union[int, str] = 0) -> int:
        """Return the indices of the end of the substring matched by group; group
        defaults to zero (meaning the whole matched substring).

        Return -1 if group exists but did not contribute to the match. For a match
        object m, and a group g that did contribute to the match, the substring matched
        by group g (equivalent to m.group(g)) is m.string[m.start(g):m.end(g)]
        """
    ...
    def get_bounding_boxes(self) -> typing.List["Rectangle"]:
        """This function returns the bounding box(es) that constitute the locations of
        the glyph(s) that matched the regular expression."""
    ...
    def group(self, __group: typing.Union[str, int] = 0) -> bytes | str:
        ...
        backreferences (\\g<1>, \\g<name>) are replaced by the contents of the corresponding group. Changed in version
        3.5: Unmatched groups are replaced with an empty string.
        """
    ...
    def span(self, __group: typing.Union[int, str] = 0) -> typing.Tuple[int, int]:
        ...
        Note that if group did not contribute to the match, this is (-1, -1). group
    ...
    def start(self, __group: typing.Union[int, str] = 0) -> int:
        """Return the indices of the start of the substring matched by group; group
        defaults to zero (meaning the whole matched substring).
...
class RegularExpressionTextExtraction(EventListener):
    """This implementation of EventListener allows you to search for regular
    expressions in a PDF Document."""
    ...
    def get_matches(self) -> typing.Dict[int, typing.List[PDFMatch]]:
        """This function returns a typing.List[PDFMatch] matching the regular
        expression."""
