...
from web.pdf.io.read.tokenize.low_level_tokenizer import Token, TokenType
from web.pdf.io.read.types import Decimal as bDecimal
from web.pdf.io.read.types import Dictionary, List, Name


class Font(Dictionary):
    """In both traditional typesetting and modern usage, the word "font" refers to the
    delivery mechanism of the typeface design.

    In traditional typesetting, the font would be made from metal or wood. Today, the
    font is a digital file.
    """
            ...
            if isinstance(self["Encoding"], Dictionary):
                ...
                if "Differences" in self["Encoding"]:
                    ...
                    for x in self["Encoding"]["Differences"]:
                        list_.append(x)
                    out["Encoding"][Name("Differences")] = list_
        # ToUnicode
                ...
                for _ in range(0, number_of_lines_003):
                    ...
                    if token.get_token_type() == TokenType.HEX_STRING:
                        ...
                        for i in range(char_code_start_003, char_code_stop_003 + 1):
                            unicode_hex: str = hex(
                                int(unicode_base_str, 16) + (i - char_code_start_003)
                            )[2:]
                            unicode_hex = (
                                "".join(
                                    ["0" for _ in range(0, 4 - len(unicode_hex) % 4)]
                                )
                                + unicode_hex
                            )
                            unicode_hex = "".join(
                                [
                                    chr(int(unicode_hex[j : j + 4], 16))
                                    for j in range(0, len(unicode_hex), 4)
                                ]
                            )
                            ...
                            unicode_hex = (
                                "".join(
                                    [
                                        "0"
                                        for _ in range(
                                            0, 4 - len(unicode_base_str_003) % 4
                                        )
                                    ]
                                )
    ...
    def get_font_name(self) -> typing.Optional[str]:
        """This function returns the name of the Font e.g. "Helvetica"."""
    ...
    def get_space_character_width_estimate(self) -> Decimal:
        """This function estimates the width of the space character (unicode 32) in
        this Font. If the Font contains the character, this Font will return the
        corresponding width.
        ...
        If all previously mentioned approaches fail, the width is estimated based on
        characters that may be present in the Font. (e.g. the width of 'A' is typically
        twice that of ' ').
        """
