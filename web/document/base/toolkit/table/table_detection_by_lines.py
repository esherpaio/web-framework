...
class TableDetectionByLines(EventListener):
    ...
    It recognizes a table whenever a substantial amount of intersecting (or connected)
    horizontal/vertical lines occur. The recognized grid must have at least 2 cells
    otherwise it could just be a piece of text in a box.
    """
    ...
    def _determine_number_of_rows_and_columns(
        lines_in_table: typing.List[LineSegment],
        ...
        if isinstance(event, EndPageEvent):
            ...
            for l0 in self._lines_per_page[self._current_page_number]:
                for l1 in self._lines_per_page[self._current_page_number]:
                    ...
                    if TableDetectionByLines._dist(
                        l0.x0, l0.y0, l1.x0, l1.y0
                    ) < Decimal(1):
                        ds.union(l0, l1)
                        continue
                    if TableDetectionByLines._dist(
                        l0.x0, l0.y0, l1.x1, l1.y1
                    ) < Decimal(1):
                        ds.union(l0, l1)
                        continue
                    if TableDetectionByLines._dist(
                        l0.x1, l0.y1, l1.x0, l1.y0
                    ) < Decimal(1):
                        ds.union(l0, l1)
                        continue
                    if TableDetectionByLines._dist(
                        l0.x1, l0.y1, l1.x1, l1.y1
                    ) < Decimal(1):
                        ds.union(l0, l1)
                        continue
                    ...
                    if (
                        abs(l0.x0 - l0.x1) <= Decimal(1)
                        and (
                            abs(l0.x0 - l1.x0) <= Decimal(1)
                            or abs(l0.x0 - l1.x1) <= Decimal(1)
                        )
                        ...
                        ds.union(l0, l1)
                    if (
                        abs(l0.y0 - l0.y1) <= Decimal(1)
                        and (
                            abs(l0.y0 - l1.y0) <= Decimal(1)
                            or abs(l0.y0 - l1.y1) <= Decimal(1)
                        )
    ...
    def get_table_bounding_boxes(self) -> typing.Dict[int, typing.List[Rectangle]]:
        """This function returns the bounding boxes (as Rectangle objects) of each
        Table that was recognized on the given page."""
