...
from web.pdf.datastructure.disjoint_set import DisjointSet
...
from web.pdf.pdf.canvas.layout.layout_element import LayoutElement
...
from web.pdf.toolkit.text.simple_line_of_text_extraction import (
    SimpleLineOfTextExtraction,
)


class SimpleParagraphExtraction(SimpleLineOfTextExtraction):
    """This implementation of EventListener extracts all paragraphs of text from a PDF
    Document."""
    ...
    def _end_page(self, page: Page):
        ...
        for line_of_text_partition in line_of_text_disjoint_set.sets():
            ...
            p._previous_layout_box = Rectangle(
                min([x.get_previous_layout_box().x for x in lines_of_text]),
                min([x.get_previous_layout_box().y for x in lines_of_text]),
                max(
                    [
                        x.get_previous_layout_box().x
                        + x.get_previous_layout_box().width
                ...
                max(
                    [
                        x.get_previous_layout_box().y
                        + x.get_previous_layout_box().height
                        for x in lines_of_text
                    ]
                )
                - min([x.get_previous_layout_box().y for x in lines_of_text]),
    ...
    @staticmethod
    def _overlap(r0: Rectangle, r1: Rectangle) -> Decimal:
    ...
    def get_paragraphs_from_pdf(
        ...
        for page_nr in range(0, number_of_pages):
            ...
            obj: "SimpleParagraphExtraction" = SimpleParagraphExtraction()
            # process Page
            ...
            paragraphs_per_page[page_nr] = obj.get_paragraphs()[0]
