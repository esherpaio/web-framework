...
class ImageFormatOptimization(EventListener):
    """This implementation of EventListener resizes all Image objects on a Page to fit
    their actual dimensions (ensuring they are not bigger than they need to be)"""
    ...
    def _render_image(self, image_render_event: "ImageRenderEvent"):
        source_image: ImageType = image_render_event.get_image()
        ...
        # resize
        if (w0 * h0) < (w1 * h1):
            resized_image: ImageType = source_image.resize((w0, h0))
