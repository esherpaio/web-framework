...
from web.pdf.pdf.canvas.color.color import Color, RGBColor
from web.pdf.pdf.canvas.font.font import Font
from web.pdf.pdf.canvas.geometry.matrix import Matrix

if TYPE_CHECKING:
    from web.pdf.pdf.canvas.color.color import RGBColor
    from web.pdf.pdf.canvas.font.font import Font
    from web.pdf.pdf.canvas.geometry.line_segment import LineSegment


class CanvasGraphicsState:
    """A conforming reader shall maintain an internal data structure called the
    graphics state that holds current graphics control parameters. These parameters
    define the global framework within which the graphics operators execute.

    EXAMPLE 1 The f (fill) operator implicitly uses the current colour parameter, and
    the S (stroke) operator additionally uses the current line width parameter from the
    graphics state.

    A conforming reader shall initialize the graphic state at the beginning of each
    page with the values specified in Table 52 and Table 53. Table 52 lists those
    graphics state parameters that are device-independent and are appropriate to
    specify in page descriptions. The parameters listed in Table 53 control details of
    the rendering (scan conversion) process and are device-dependent; a page
    description that is intended to be device- independent should not be written to
