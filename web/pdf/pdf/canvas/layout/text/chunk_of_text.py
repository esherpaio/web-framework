...
class ChunkOfText(LayoutElement):
    """This implementation of LayoutElement represents one uninterrupted block of
    text."""
    ...
    @staticmethod
    def _get_font_resource_name(font: Font, page: Page):
    ...
    @staticmethod
    def _pad_string_with_zeroes(s: str, n: int = 2) -> str:
        while len(s) < n:
            s = "0" + s
        return s
    ...
    def _write_text_bytes_in_ascii(self) -> str:
        ...
        for c in self._text:
            if c == "\r":
                ...
                s_out += "\\00" + oct(ord(c))[2:]
            elif 8 <= ord(c) < 32:
                s_out += "\\0" + oct(ord(c))[2:]
            else:
                s_out += c
        # default
        return "".join(["(", s_out, ") Tj"])
