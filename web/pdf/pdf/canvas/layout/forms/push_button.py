...
from web.pdf.pdf.canvas.font.simple_font.font_type_1 import StandardType1Font
from web.pdf.pdf.canvas.geometry.rectangle import Rectangle
from web.pdf.pdf.canvas.layout.forms.form_field import FormField
...
class PushButton(FormField):
    ...
    def _init_widget_dictionary(self, page: Page) -> None:
        ...
        self._widget_dictionary[Name("AP")][Name("N")][Name("BBox")] = (
            List().set_is_inline(True)
        )
        for _ in range(0, 4):
            self._widget_dictionary[Name("AP")][Name("N")][Name("BBox")].append(
                bDecimal(0)
            )
        self._widget_dictionary[Name("AP")][Name("N")][Name("DecodedBytes")] = (
            b"/Tx BMC EMC"
        )
        self._widget_dictionary[Name("AP")][Name("N")][Name("Bytes")] = zlib.compress(
            self._widget_dictionary[Name("AP")][Name("N")][Name("DecodedBytes")], 9
        )
        self._widget_dictionary[Name("AP")][Name("N")][Name("Filter")] = Name(
            "FlateDecode"
        )
        self._widget_dictionary[Name("AP")][Name("N")][Name("Length")] = bDecimal(
            len(self._widget_dictionary[Name("AP")][Name("N")][Name("Bytes")])
        )
        self._widget_dictionary[Name("AP")][Name("N")][Name("Resources")] = Dictionary()
        self._widget_dictionary[Name("AP")][Name("N")][Name("Resources")][
            Name("ProcSet")
        ] = List().set_is_inline(True)
        self._widget_dictionary[Name("AP")][Name("N")][Name("Resources")][
            Name("ProcSet")
        ...
        self._widget_dictionary[Name("DA")] = String(
            "0.23921 0.23921 0.23921 rg /%s %f Tf"
            % (font_resource_name, float(self._font_size))
        )
    ...
    def _paint_content_box(self, page: "Page", available_space: Rectangle) -> None:
        ...
        if self._widget_dictionary is not None:
            ...
            self._widget_dictionary["Rect"][2] = bDecimal(
                cbox.get_x() + cbox.get_width()
            )  # ur_x
            self._widget_dictionary["Rect"][3] = bDecimal(
                cbox.get_y() + cbox.get_height()
            )  # ur_y
...
class JavaScriptPushButton(PushButton):
    """This implementation of FormField represents a push button that triggers
    JavaScript."""
    ...
    def _init_widget_dictionary(self, page: Page) -> None:
        ...
        if self._widget_dictionary is not None:
            ...
            self._widget_dictionary[Name("AA")][Name("D")][Name("JS")] = (
                javascript_stream
            )
