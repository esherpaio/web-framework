...
from web.pdf.pdf.canvas.layout.equation.parser import Parser
from web.pdf.pdf.canvas.layout.equation.token import Token, TokenType
...
from web.pdf.pdf.canvas.layout.table.table import Table, TableCell
from web.pdf.pdf.canvas.layout.text.chunk_of_text import ChunkOfText
from web.pdf.pdf.canvas.layout.text.paragraph import Paragraph


class Equation(InlineFlow):
    """In mathematics, an equation is a formula that expresses the equality of two
    expressions, by connecting them with the equals sign =.

    The word equation and its cognates in other languages may have subtly different
    meanings; for example, in French an équation is defined as containing one or more
    variables, while in English, any well-formed formula consisting of two expressions
    related with an equals sign is an equation.
    """
    ...
    def _build(
        ...
        if t.get_type() == TokenType.OPERATOR:
            ...
            if t.get_text() == "-" and t.get_number_of_arguments() == 1:
                out = FlexibleColumnWidthTable(number_of_columns=2, number_of_rows=1)
                out.add(
                    ChunkOfText(
                        "-",
                        font_size=font_size,
            ...
            if t.get_text() == "/":
                ...
                out = FlexibleColumnWidthTable(number_of_columns=1, number_of_rows=2)
                out.add(
                    TableCell(
                        e1,
                        border_top=False,
            ...
            if t.get_text() == "^":
                ...
                e1._horizontal_alignment = Alignment.CENTERED
                out = FlexibleColumnWidthTable(number_of_columns=2, number_of_rows=2)
            ...
            out = FlexibleColumnWidthTable(number_of_columns=3, number_of_rows=1)
            out.add(c1)
            out.add(
                ChunkOfText(
                    op_text,
