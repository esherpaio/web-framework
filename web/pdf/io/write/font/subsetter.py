...
from web.pdf.io.write.font.character_set_listener import CharacterSetListener
from web.pdf.io.write.font.copy_command_operator import CopyCommandOperator
from web.pdf.io.write.font.subset_show_text_with_glyph_positioning import (
...
from web.pdf.pdf.canvas.font.simple_font.true_type_font import TrueTypeFont
...
class Subsetter:
    ...
    Subsetting is performed using fontTools.subset, after which the dictionaries are
    updated in the Type0Font, the page content is also changed, updating the TJ
    instructions to refer to the new GIDs.
    """
    ...
    def apply(page: Page) -> Page:
        """
        This function applies Font subsetting to the input Page and returns the Page
        afterwards
        ...
        for old_font in fonts_to_be_subset:
            ...
            new_font[Name("BaseFont")] = Name(
                basefont_prefix + "+" + str(new_font[Name("BaseFont")])
            )
            new_font["FontDescriptor"][Name("FontName")] = new_font[Name("BaseFont")]
            new_font[Name("ToUnicode")] = (
                TrueTypeFont._build_custom_cmap_for_type_0_font(ttfont)
            )
