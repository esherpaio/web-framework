...
class CopyCommandOperator(CanvasOperator):
    """This CanvasOperator copies an existing operator and writes its bytes to the
...
class ShowTextWithGlyphPositioningMod(CanvasOperator):
    """This operator represents a modified version of the TJ operator Instead of always
    rendering the text, it takes into account the location at which the text is to be
    rendered, if the text falls in one of the redacted areas it will not render the
    text."""
...
class RedactedCanvasStreamProcessor(CanvasStreamProcessor):
    """In computer science and visualization, a canvas is a container that holds
    various drawing elements (lines, shapes, text, frames containing other elements,
    etc.).

    It takes its name from the canvas used in visual arts. This implementation of
    Canvas automatically handles redaction (removal of content).
    """
    ...
    def get_redacted_content(self) -> bytes:
        """This function returns the redacted content of this implementation of
