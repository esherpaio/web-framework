...
from web.pdf.io.read.encryption.standard_security_handler import (
...
from web.pdf.pdf.xref.rebuilt_xref import RebuiltXREF
from web.pdf.pdf.xref.stream_xref import StreamXREF
from web.pdf.pdf.xref.xref import XREF


class BeginDocumentEvent(Event):
    """This implementation of Event is triggered right before the Document is
    processed."""
...
class EndDocumentEvent(Event):
    """This implementation of Event is triggered right after the Document was
    processed."""
...
class XREFTransformer(Transformer):
    """This implementation of ReadBaseTransformer is responsible for reading the XRef
    object."""
    ...
    def _read_xref(
        ...
        :param context:         the TransformerContext (containing the io source)
        :type context:          TransformerContext
        :param initial_offset:  the initial byte offset at which to read
        ...
        # attempt to rebuild XREF from web.pdfument
        if most_recent_xref is None:
            try:
    ...
    def transform(
        ...
        for listener in event_listeners:
            listener._event_occurred(BeginDocumentEvent())
        ...
        if "Trailer" in xref and "Encrypt" in xref["Trailer"]:
            ...
            xref["Trailer"][Name("Encrypt")] = self.get_root_transformer().transform(
                xref["Trailer"]["Encrypt"], xref["Trailer"], context, event_listeners
            )
        ...
        # notify
        for listener in event_listeners:
            listener._event_occurred(EndDocumentEvent())
