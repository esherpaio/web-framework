...
from web.pdf.io.read.types import Dictionary, Name
from web.pdf.pdf.canvas.font.adobe_glyph_list import AdobeGlyphList
from web.pdf.pdf.canvas.font.adobe_standard_encoding import (
    adobe_standard_decode,
    adobe_standard_encode,
)
from web.pdf.pdf.canvas.font.font import Font
from web.pdf.pdf.canvas.font.simple_font.simple_font import SimpleFont
from web.pdf.pdf.canvas.font.symbol_encoding import (
    symbol_decode,
    zapfdingbats_decode,
)
...
class Type1Font(SimpleFont):
    ...
    information that enables high-quality rendering even at small sizes and low
    resolutions.
    """
    ...
    def _read_encoding_with_differences(self) -> None:
        ...
        # AND those glyph names all resolve to <empty string>
    ...
    def character_identifier_to_unicode(
        ...
        # If the font dictionary contains a ToUnicode CMap (see 9.10.3,
        ...
        # only character names taken from the Adobe standard Latin character set and
        # the set of named characters in the Symbol font (see Annex D)
        # a) Map the character code to a character name according to Table D.1 and the
        # font’s Differences  array.
        # b) Look up the character name in the Adobe Glyph List (see the Bibliography)
...
class StandardType1Font(Type1Font):
    ...
    follows: Times-Roman, Helvetica, Courier, Symbol, Times-Bold, Helvetica-Bold,
    Courier-Bold, ZapfDingbats, Times-Italic, Helvetica- Oblique, Courier-Oblique,
    Times-BoldItalic, Helvetica-BoldOblique, Courier- BoldOblique These fonts, or their
    font metrics and suitable substitution fonts, shall be available to the conforming
    reader.
    """
    ...
    def __init__(self, font_name: typing.Optional[str] = None):
        ...
        if font_name is not None:
            ...
            if font_name == "Symbol":
                self._character_identifier_to_unicode_lookup = {
                    c: symbol_decode(bytes([c])) for c in range(0, 256)
                }
                self._unicode_lookup_to_character_identifier = {
                    v: k
                ...
                self._character_identifier_to_unicode_lookup = {
                    c: zapfdingbats_decode(bytes([c])) for c in range(0, 256)
                }
                self._unicode_lookup_to_character_identifier = {
                    v: k
                    for k, v in self._character_identifier_to_unicode_lookup.items()
                ...
                for c in range(0, 256):
                    try:
                        self._character_identifier_to_unicode_lookup[c] = bytes(
                            [c]
                        ).decode("cp1252")
                    except:  # noqa: E722
    ...
    def is_standard_14_font_name(font_name: str) -> bool:
        """This function returns True if the given str represents the name of one of
        the standard 14 fonts, False otherwise."""
