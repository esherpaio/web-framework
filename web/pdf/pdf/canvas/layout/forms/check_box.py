...
from web.pdf.io.read.types import Boolean, Dictionary, Name, Stream, String
from web.pdf.io.read.types import Decimal as bDecimal
from web.pdf.io.read.types import List as bList
from web.pdf.io.read.types import String as bString
from web.pdf.pdf.canvas.color.color import Color, HexColor, RGBColor
    ...
    def _paint_content_box(self, page: "Page", content_box: Rectangle) -> None:
        ...
        if self._widget_dictionary is not None:
            ...
            self._widget_dictionary["Rect"][1] = bDecimal(
                content_box.get_y() + content_box.height - line_height
            )
            self._widget_dictionary["Rect"][2] = bDecimal(
                content_box.get_x() + content_box.width
            )
            self._widget_dictionary["Rect"][3] = bDecimal(
                content_box.get_y() + line_height
            )
