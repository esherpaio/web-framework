...
from web.pdf.io.read.types import List, Name, Stream
...
class Type0Font(Font):
    ...
    A composite font shall be represented by a font dictionary whose Subtype value is
    Type0. The Type 0 font is known as the root font, and its associated CIDFont is
    ...
    def _find_best_matching_predefined_cmap(cmap_name: str) -> typing.Dict[int, str]:
        ...
        if cmap_name not in predefined_cmaps:
            ...
            elif cmap_name == "Adobe-Japan1-UCS2":
                cmap_name = "Adobe-Japan1-0"
            elif cmap_name not in predefined_cmaps:
                cmap_name = "Adobe-Identity-H"
    ...
    def _get_cmap_name(self) -> str:
        # b) Obtain the registry and ordering of the character collection used by the
        ...
        # c) Construct a second CMap name by concatenating the registry and ordering
        # obtained in step (b) in the format registry–ordering–UCS2 (for example,
        # Adobe–Japan1–UCS2).
    ...
    def character_identifier_to_unicode(
        ...
        if Name("Encoding") in self:
            # a) Map the character code to a character identifier (CID) according to
            # the font’s CMap.
            ...
            # d) Obtain the CMap with the name constructed in step (c) (available from
            # the ASN Web site; see the Bibliography).
            ...
            # e) Map the CID obtained in step (a) according to the CMap obtained in
            # step (d), producing a Unicode value.
