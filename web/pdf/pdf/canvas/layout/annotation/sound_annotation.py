...
from web.pdf.io.read.types import Boolean, Dictionary, Name, String
...
class SoundAnnotation(Annotation):
    """A sound annotation (PDF 1.2) shall analogous to a text annotation except that
    instead of a text note, it contains sound recorded from the computer’s microphone
    or imported from a file.

    When the annotation is activated, the sound shall be played. The annotation shall
    behave like a text annotation in most ways, with a different icon (by default, a
    speaker) to indicate that it represents a sound. Table 185 shows the annotation
    dictionary entries specific to this type of annotation. Sound objects are discussed
    in 13.3, “Sounds.”
    """
    ...
    def __init__(self, bounding_box: Rectangle, url_to_mp3_file: str):
        ...
        self[Name("A")][Name("R")][Name("C")][Name("D")][Name("Type")] = Name(
            "Filespec"
        )
        self[Name("A")][Name("R")][Name("C")][Name("D")][Name("FS")] = Name("URL")
        self[Name("A")][Name("R")][Name("C")][Name("D")][Name("F")] = String(
            SoundAnnotation._make_canonical_file_path(self._url_to_mp3_file)
        )

        # A/R/C/P

        self[Name("A")][Name("R")][Name("C")][Name("P")] = Dictionary()
        self[Name("A")][Name("R")][Name("C")][Name("P")][Name("TF")] = String(
            "TEMPACCESS"
        )
