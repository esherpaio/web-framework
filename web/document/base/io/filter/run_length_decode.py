...
class RunLengthDecode:
    ...
    reproducing the original text or binary data (typically monochrome image data, or
    any data that contains frequent long runs of a single byte value)."""
    ...
    def decode(bytes_in: bytes) -> bytes:
        ...
        # byte shall be copied 257 - length (2 to 128) times during decompression. A
        # length value of 128 shall denote EOF.
        bytes_out = bytearray()
        i: int = 0
        while i < len(bytes_in):
            ...
            if b == 128:
                break
            # If the length byte is in the range 0 to 127, the following length + 1
            # (1 to 128) bytes shall be copied literally during decompression.
            if 0 <= b <= 127:
                length = b + 1
                i += 1
