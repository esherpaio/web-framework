...
from web.pdf.pdf.canvas.layout.table.table import Table
...
class FlexibleColumnWidthTable(Table):
    """This class represents a Table with columns that will assume a width based on
    their contents.

    It tries to emulate the behaviour of <table> elements in HTML
    """
    ...
    def _get_grid_coordinates(
        ...
        #       cell with the largest minimum cell width. The maximum is that required
        #       by the cell with the largest maximum cell width.
        ...
        #       they are at least as wide as the column group's 'width'. This gives a
        #       maximum and minimum width for each column.

        # 5.    calculate column width based on min, max and bounding box start by
        #       assigning each column its minimum width
        column_widths: typing.List[Decimal] = [x for x in min_column_widths]

        # as long as there are columns that could be expanded, and there is room to
        # expand them add Decimal(1) to each (expandable) column_width
        ...
        for r in range(0, self._number_of_rows):
            ...
            for e in [x for x in self._get_cells_at_row(r) if x._row_span == 1]:
                grid_x = min([p[1] for p in e._table_coordinates])
