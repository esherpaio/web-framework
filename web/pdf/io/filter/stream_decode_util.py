...
from web.pdf.io.filter.ascii85_decode import ASCII85Decode
from web.pdf.io.filter.flate_decode import FlateDecode
from web.pdf.io.filter.lzw_decode import LZWDecode
from web.pdf.io.filter.run_length_decode import RunLengthDecode
from web.pdf.io.read.types import Decimal, Dictionary, List, Name, Stream


def decode_stream(s: Stream) -> Stream:
    """This function decodes a Stream, applying the filters specified in the Filter
    ...
    if "DecodeParms" in s:
        ...
        decode_params = [Dictionary() for _ in range(0, len(filters))]
