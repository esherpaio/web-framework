...
from web.pdf.pdf.canvas.event.image_render_event import ImageRenderEvent
from web.pdf.pdf.canvas.geometry.rectangle import Rectangle
from web.pdf.pdf.pdf import PDF


class FreeSpaceFinder(EventListener):
    """This implementation of EventListener keeps track of which space on a Page is
    available."""

    class Grid:
        """This class represents a rasterized, low-res view of a Page.

        But rather than rendering instructions on this raster, each cell simply keeps
        track of whether or not the cell is available. This enables a quick lookup for
        availability of a given Rectangle.
        """
        ...
        def _get_free_space(
            ...
            """This function returns a Rectangle (or None) of free space (no text
            rendering operations, no drawing operations) near the given Rectangle."""
            ...
            for i in range(0, len(self._availability) - w):
                for j in range(0, len(self._availability[i]) - h):
                    ...
                    for k in range(0, w):
                        for l1 in range(0, h):
                            if not self._availability[i + k][j + l1]:
                                is_free = False
                                break
                        if not is_free:
                            break
    ...
    def _event_occurred(self, event: Event) -> None:
        ...
        if isinstance(event, ChunkOfTextRenderEvent):
            ...
            bounding_box_001: typing.Optional[Rectangle] = (
    ...
    def find_free_space_for_page(
        ...
        """This function returns the nearest (euclidean distance) empty Rectangle that
        is at least as wide and tall as the desired Rectangle.

        If no such Rectangle exists, this method returns None.
        """

        obj: FreeSpaceFinder = FreeSpaceFinder()
        with open(file, "rb") as pdf_file_handle:
            PDF.loads(pdf_file_handle, [obj])
        return obj.get_free_space_for_page(page_number, desired_rectangle)
