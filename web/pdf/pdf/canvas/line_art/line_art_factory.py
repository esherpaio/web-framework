...
from web.pdf.pdf.canvas.geometry.rectangle import Rectangle
from web.pdf.pdf.canvas.line_art.blob_factory import BlobFactory

HALF: Decimal = Decimal(0.5)


class LineArtFactory:
    """This class provides static convenience methods for generating points of common
    line- art in electronic documents, such as arrows, rectangles, triangles, regular
    n-gons, stars, etc."""
    ...
    def eurion(
        ...
        """The EURion constellation (also known as Omron rings or doughnuts) is a
        pattern of symbols incorporated into a number of secure documents such as
        banknotes and ownership title certificates designs worldwide since about 1996.

        It is added to help imaging software detect the presence of such a document in
        a digital image. Such software can then block the user from reproducing
        banknotes to prevent counterfeiting using colour photocopiers.
        According to research from 2004, the EURion constellation is used for colour
        photocopiers but probably not used in computer software.
        It has been reported that Adobe Photoshop will not allow editing of an image of
        a banknote, but in some versions this is believed to be due to a different,
        unknown digital watermark rather than the EURion constellation.
        :return:
        """
        ...
        # scale
        min_x: Decimal = min([min(x[0][0], x[1][0]) for x in line_segments])
        max_x: Decimal = max([max(x[0][0], x[1][0]) for x in line_segments])
        w: Decimal = max_x - min_x
        min_y: Decimal = min([min(x[0][1], x[1][1]) for x in line_segments])
        max_y: Decimal = max([max(x[0][1], x[1][1]) for x in line_segments])
    ...
    def arrow_down(bounding_box: Rectangle) -> typing.List[Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for an arrow pointing down that fits
    ...
    def arrow_left(bounding_box: Rectangle) -> typing.List[Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for an arrow pointing left that fits
        in the given bounding box."""

        return [
            (bounding_box.x, bounding_box.y + bounding_box.width * Decimal(0.5)),
            (bounding_box.x + bounding_box.width * Decimal(0.39), bounding_box.y),
            (
    ...
    def arrow_right(bounding_box: Rectangle) -> typing.List[Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for an arrow pointing right that fits
        in the given bounding box."""
        ...
        return [
            ...
            (bounding_box.x, bounding_box.y + bounding_box.height * Decimal(0.8)),
            (bounding_box.x, bounding_box.y + bounding_box.height * Decimal(0.2)),
            (
                bounding_box.x + bounding_box.width * Decimal(0.61),
                bounding_box.y + bounding_box.height * Decimal(0.2),
            ),
            (bounding_box.x + bounding_box.width * Decimal(0.61), bounding_box.y),
            # repeat first point to explicitly close shape
            (
                bounding_box.x + bounding_box.width,
                bounding_box.y + bounding_box.width * Decimal(0.5),
    ...
    def arrow_up(bounding_box: Rectangle) -> typing.List[Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for an arrow pointing up that fits in
    ...
    def cartoon_diamond(
        ...
        """This function returns the coordinates for a cartoon diamond that matches the
        given bounding box."""
    ...
    def circle(bounding_box: Rectangle) -> typing.List[typing.Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for a circle that fits in the given
        bounding box."""
    ...
    def cross(bounding_box: Rectangle) -> typing.List[Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for a cross that matches the given
        bounding box."""
    ...
    def diamond(bounding_box: Rectangle) -> typing.List[Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for a diomond that fits in the given
        bounding box."""
        ...
        return [
            ...
            (bounding_box.x, bounding_box.y + bounding_box.height * HALF),
            # repeat first point to explicitly close shape
            (bounding_box.x + bounding_box.width * HALF, bounding_box.y),
        ]
    ...
    def dragon_curve(
        ...
        """This function returns the coordinates for the Heighway dragon (also known as
        the Harter–Heighway dragon, or the Jurassic Park dragon) curve that fits in the
        given bounding box."""
    ...
    def droplet(bounding_box: Rectangle) -> typing.List[Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for a droplet that fits in the given
        bounding box."""
    ...
    def five_pointed_star(
        ...
        """This function returns the coordinates for a five point star that fits in the
        given bounding box."""
    ...
    def flowchart_card(
        ...
        """This is the old IBM punched card.

        Each line of a program was punched into one IBM card. Then the cards were
        stacked in order and taken to a card reader. Usually the student would submit
        the cards and someone else would run them during the middle of the night, when
        the computer wasn't so busy. The output was printed on wide z-fold paper. If
        you made a mistake, you would have to resubmit the cards and wait another day.
        Large programs had stacks of cards several feet high. If you are using this
        shape, you need to update your hardware.
        """
    ...
    def flowchart_data(
        ...
        """Indicates the process of inputting and outputting data, as in entering data
    ...
    def flowchart_decision(
        ...
        The operation is commonly a yes/no question or true/false test. Represented as
        a diamond (rhombus).
        """
    ...
    @staticmethod
    def flowchart_direct_data() -> typing.List[typing.Tuple[Decimal, Decimal]]:
        """Direct Data object in a process flow represents information stored which can
        be accessed directly.

        This object represents a computer's hard drive.
        """
        """This is a general data storage object used in the process flow as opposed to
        data which could be also stored on a hard drive, magnetic tape, memory card, of
    ...
    def flowchart_extract(
        ...
        """The Extract shape involves removal of one or more specific sets of items
        from a set.

        For example, you could have a list of addresses and extract those that are
        within 10 miles of some location.
        """
    ...
    def flowchart_internal_storage(
        ...
        """This is a shape which is commonly found in programming flowcharts to
        illustrate the information stored in memory, as opposed to on a file.

        This shape is often referred to as the magnetic core memory of early computers;
    ...
    def flowchart_manual_input(
        ...
        """Represented by quadrilateral, with the top irregularly sloping up from left
        to right, like the side view of a keyboard.
    ...
    def flowchart_manual_operation(
        ...
        """Represented by a trapezoid with the longest parallel side at the top, to
        represent an operation or adjustment to process that can only be made
        manually."""
    ...
    @staticmethod
    def flowchart_multiple_documents() -> typing.List[typing.Tuple[Decimal, Decimal]]:
    ...
    def flowchart_on_page_reference(
        ...
        """This small circle (also known as Connector) indicates that the next (or
        previous) step is somewhere else on the drawing.

        This is particularly useful for large flowcharts where you would otherwise have
        to use a long connector, which can be hard to follow.
        """
    ...
    def flowchart_or(
        ...
        """Just as described, this shape indicates that the process flow continues two
        paths or more."""
    ...
    def flowchart_preparation(
        ...
        """Represented by an elongated hexagon, originally used for steps like setting
    ...
    def flowchart_sequential_data(
        ...
        """This shape is supposed to look like a reel of tape with a small portion of
        tape extending from the reel.

        It represents magnetic tape storage which is also called sequential access
        storage.
        """
    ...
    def flowchart_termination(
        ...
        contain the word "Start" or "End", or another phrase signaling the start or end
    ...
    @staticmethod
    def flowchart_transport() -> typing.List[typing.Tuple[Decimal, Decimal]]:
        """The Transport flowchart shape represents the step in the flowchart where
        information or materials are being transported from the process."""
    ...
    def four_pointed_star(
        ...
        """This function returns the coordinates for a four point star that fits in the
        given bounding box."""
    ...
    def fraction_of_circle(
        bounding_box: Rectangle, fraction: Decimal
    ) -> typing.List[Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for a circle-slice that fits in the
        given bounding box."""
    ...
    def half_of_circle(bounding_box: Rectangle) -> typing.List[Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for a circle-slice of 180 degrees that
        fits in the given bounding box."""
    ...
    def heart(bounding_box: Rectangle) -> typing.List[Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for a heart that fits in the given
        bounding box."""
    ...
    def heptagon(bounding_box: Rectangle) -> typing.List[Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for a heptagon that fits in the given
        bounding box."""
    ...
    def hexagon(bounding_box: Rectangle) -> typing.List[Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for a hexagon that fits in the given
        bounding box."""
    ...
    def isosceles_triangle(
        ...
        """This function returns the coordinates for an isosceles triangle that fits in
        the given bounding box."""
    ...
    def lissajours(
        ...
        """A Lissajous curve /ˈlɪsəʒuː/, also known as Lissajous figure or Bowditch
        curve /ˈbaʊdɪtʃ/, is the graph of a system of parametric equations which
        describe complex harmonic motion.

        This family of curves was investigated by Nathaniel Bowditch in 1815, and later
        in more detail in 1857 by Jules Antoine Lissajous (for whom it has been named).
        The appearance of the figure is highly sensitive to the ratio x_frequency /
        y_frequency. For a ratio of 1, the figure is an ellipse, with special cases
        including circles. The visual form of these curves is often suggestive of a
        three-dimensional knot, and indeed many kinds of knots, including those known
        as Lissajous knots, project to the plane as Lissajous figures.
        """
    ...
    def n_pointed_star(
        ...
        """This function returns the coordinates for an n-point star that fits in the
        given bounding box."""
    ...
    def octagon(bounding_box: Rectangle) -> typing.List[Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for an octagon that fits in the given
        bounding box."""
    ...
    def parallelogram(bounding_box: Rectangle) -> typing.List[Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for a parallelogram that fits in the
        given bounding box."""

        return [
            (bounding_box.x, bounding_box.y),
            (bounding_box.x + bounding_box.width * Decimal(0.75), bounding_box.y),
            (bounding_box.x + bounding_box.width, bounding_box.y + bounding_box.height),
            (
    ...
    def pentagon(bounding_box: Rectangle) -> typing.List[Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for a pentagon that fits in the given
        bounding box."""
    ...
    def rectangle(bounding_box: Rectangle) -> typing.List[Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for a rectangle that matches the given
        bounding box."""
    ...
    def regular_n_gon(
        ...
        """This function returns the coordinates for a regular n-gon that fits in the
        given bounding box."""
    ...
    def right_angled_triangle(
        ...
        """This function returns the coordinates for a right-angled triangle that fits
        in the given bounding box."""
    ...
    def six_pointed_star(
        ...
        """This function returns the coordinates for a six point star that fits in the
        given bounding box."""
    ...
    def sticky_note(bounding_box: Rectangle) -> typing.List[Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for a sticky note that fits in the
        given bounding box."""
    ...
    def three_quarters_of_circle(
        ...
        """This function returns the coordinates for a circle-slice of 270 degrees that
        fits in the given bounding box."""
    ...
    def trapezoid(bounding_box: Rectangle) -> typing.List[Tuple[Decimal, Decimal]]:
        """This function returns the coordinates for a trapezoid that fits in the given
        bounding box."""
