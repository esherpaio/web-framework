...
from web.pdf.io.read.postfix.postfix_eval import PostScriptEval
...
class Decimal(PDFObject, oDecimal):
    """PDF provides two types of numeric objects: integer and real.

    Integer objects represent mathematical integers. Real objects represent
    mathematical real numbers. The range and precision of numbers may be limited by the
    internal representations used in the computer on which the conforming reader is
    running; Annex C gives these limits for typical implementations.
    """
...
class Dictionary(PDFObject, dict):
    """A dictionary object is an associative table containing pairs of objects, known
    as the dictionary’s entries. The first element of each entry is the key and the
    second element is the value. The key shall be a name (unlike dictionary keys in
    PostScript, which may be objects of any type). The value may be any kind of object,
    including another dictionary. A dictionary entry whose value is null (see 7.3.9,
    "Null Object") shall be treated the same as if the entry does not exist. (This
    differs from PostScript, where null behaves like any other object as the value of a
    dictionary entry.) The number of entries in a dictionary shall be subject to an
    implementation limit; see Annex C. A dictionary may have zero entries.
...
class Function(Dictionary):
    ...
    The term function dictionary is used generically in this sub-clause to refer to
    either a dictionary object or the dictionary portion of a stream object. A function
    dictionary specifies the function’s representation, the set of attributes that
    parameterize that representation, and the additional data needed by that
    ...
    def _get_sample_number(self, sample: typing.List[oDecimal]) -> typing.Optional[int]:
        ...
        for s in size:
            n1 *= s
        f: typing.List[int] = [int(n1 / size[0])]
    ...
    def evaluate(self, xs: typing.List[oDecimal]) -> typing.List[oDecimal]:
        """This function evaluates this Function in the given arguments, returning a
        typing.List[Decimal] as output."""
        ...
        if "FunctionType" in self and int(self["FunctionType"]) == 0:
            ...
            # When a sampled function is called, each input value xi , for 0 £ i < m,
            # shall be clipped to the domain:
            xs_prime = [
            ...
            # That value shall be clipped to the size of the sample table in that
            # dimension:
            es_prime = [
                min(max(es[i], oDecimal(0)), size[i] - 1) for i in range(0, len(es))
            ]

            # The encoded input values shall be real numbers, not restricted to
            # integers. Interpolation shall be used to determine output values from
            # the nearest surrounding values in the sample table.
        ...
        # Type 2 functions (PDF 1.3) include a set of parameters that define an
        # exponential interpolation of one input value and n output values:
        if "FunctionType" in self and int(self["FunctionType"]) == 2:
            ...
            return [(c0[j] + xs[0] ** n2 * (c1[j] - c0[j])) for j in range(0, n)]
...
class List(PDFObject, list):
    ...
    that is, an array’s elements may be any combination of numbers, strings,
    dictionaries, or any other objects, including other arrays. An array may have zero
    elements.
    """
...
class Name(PDFObject):
    ...
    characters denote the same object. Atomic means that a name has no internal
    structure; although it is defined by a sequence of characters, those characters are
    not considered elements of the name.
    """
...
class Stream(Dictionary):
    ...
    Furthermore, a stream may be of unlimited length, whereas a string shall be subject
    to an implementation limit. For this reason, objects with potentially large amounts
    of data, such as images and page descriptions, shall be represented as streams.
    """
...
class String(PDFObject):
    ...
    Any characters may appear in a string except unbalanced parentheses (LEFT
    PARENHESIS (28h) and RIGHT PARENTHESIS (29h)) and the backslash (REVERSE SOLIDUS
    (5Ch)), which shall be treated specially as described in this sub-clause. Balanced
    ...
    def __init__(self, bts: typing.Union[bytes, str]):
        super().__init__()
    ...
    def get_content_bytes(self) -> bytearray:
        """This function returns the bytes that represent the *interpreted* content (as
        it was present in the PDF) of this String.

        For hexadecimal Strings, this content includes resolving the hexadecimal codes
        to their byte counterparts.
        """
    ...
    def get_value_bytes(self):
        """This function returns the bytes that represent the content (as it was
        present in the PDF) of this String."""
...
class HexadecimalString(String):
    ...
    either A–F or a–f) encoded as ASCII characters and enclosed within angle brackets
